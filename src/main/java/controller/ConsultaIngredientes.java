...
public class ConsultaIngredientes extends HttpServlet{
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			if(idIngredientes == null || idIngredientes.isEmpty()) {
				
				listaIngredientes = ingredienteServiceImpl.findAll();
				
			} else {
				String [] ids = idIngredientes.split(";");
				
				for (String i: ids) {
					
					Ingrediente ingredientePesquisado = ingredienteServiceImpl.findById(Integer.parseInt(i));
					
					listaIngredientes.add(ingredientePesquisado);
				}
			}
